...
#----------------------------------------#
def publish_dtm():
    return (dt.datetime.now()+dt.timedelta(hours=5.5)).strftime('%d %b %I%p')
...
#----------------------------------------#
# function for creating trend tweet
def trend_txt(trend_df):
    
    twt_template = param.trend_template
    ...
    if deltaconfirmed_value > 0:
        tweet_back = twt_template.replace('::publish_dtm::', publish_dtm())
        # Trend values
        tweet_back = tweet_back.replace('::deltaconfirmed_value::', str(int(deltaconfirmed_value)))
        tweet_back = tweet_back.replace('::deltaconfirmed_trend::', str(deltaconfirmed_trend))
        tweet_back = tweet_back.replace('::deltarecovered_value::', str(int(deltarecovered_value)))
